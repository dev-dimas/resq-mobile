...
export default function ProductDetail() {
  ...
  return (
    <>
      ...
      />
      <UserLayout containerClassname="px-0 mb-[106px]">
        <ImageViewer
          title="Foto Produk"
        ...
        <View className="w-full mt-[14px] px-6 rounded-t-[20px] pt-5 bg-white flex-1 min-h-full border border-slate-200 border-opacity-50">
          ...
          <Skeleton isLoading={isPending} height={32} width={100}>
            <View className="flex-row items-center my-1 gap-x-1">
              <Octicons name="clock" size={18} color="black" />
              <Text
                className="mb-1 text-sm font-pjs-semibold"
                style={{ includeFontPadding: false }}
              >
                {dayjs(data?.data.product.startTime).format("HH:mm")} -{" "}
                {dayjs(data?.data.product.endTime).format("HH:mm")}
              </Text>
            </View>
          </Skeleton>

          {/* Seller information */}
          <View className="flex flex-row items-center pt-1 pb-3">
            <TouchableOpacity
          ...
          {isPending ? (
            <>
              <Skeleton isLoading={isPending} height={20} width={"100%"} marginBottom={4}>
                <></>
              </Skeleton>
              <Skeleton isLoading={isPending} height={20} width={"100%"} marginBottom={4}>
                <></>
              </Skeleton>
              <Skeleton isLoading={isPending} height={20} width={"50%"} marginBottom={4}>
                <></>
              </Skeleton>
            </>
          ) : (
      ...
      <View className="w-full h-[106px] border-t border-slate-200 border-opacity-50 flex flex-row justify-between items-center p-6 pb-9 absolute bottom-0 bg-white">
